...
*图源：论文 Figure 3（Visual Object Replacement Attack）。*
...
*图源：论文 Figure 2，展示 Temporal Continuity、Temporal Jump、Causality 等九类多图关系样本。*
...
*图源：论文 Figure 3，可同时看到外部文本、表格、`R1/R2/R3` 引用链和目标行。*
...
![Jailbreak-AudioBench 的数据与音频编辑框架](images/04_jailbreak_audiobench_pipeline.png)

*图源：论文 Figure 1，展示基础数据、音频编辑工具箱、目标音频模型和结果判定流程。*
...
![Alignment Curse 的文本到音频攻击迁移流程](images/05_alignment_curse_transfer.png)

*图源：论文 Figure 1，展示文本攻击经 TTS 迁移到音频通道的机制。*
...
![PJ-Break 的六种语音表达条件](images/06_pj_break_presets.png)

*图源：论文 Figure 1，固定 transcript，只改变 Neutral、Panic、Anger、Commanding、Fast 和 Whisper 等表达条件。*
...
![SSJ 的关键词拼读与占位符提示模板](images/07_ssj_spelling_example.png)

*图源：论文官方仓库 `resources/Jailbreak_template.png`，展示逐字母音频与 `[word]` 文本占位符的联合输入模板。*
...
![AIA 的文本与干扰音频联合攻击流程](images/08_aia_pipeline.png)

*图源：论文 Figure 4，展示文本攻击失败后按排名逐个追加干扰音频的流程。*
...
*图源：论文 Figure 3，以同一条种子展示纯文本、图像、视频、音频、双模态和全模态格式。*
...
![AdvBench-Omni 的单模态与多模态构造流程](images/10_advbench_omni_construction.png)

*图源：论文 Figure 1，展示直接渲染与语义拆分两条构造路线及 11 个模态子集。*
...
*图源：DeepKeep InkJect 技术博客的官方示例图。第一张是低对比度文字载荷，第二张是旋转和透视扭曲版本。*
...
*图源：Trail of Bits 博客 Figure 1 和 Figure 5，分别展示缩放前后差异与 Anamorpher 的交互式对比。*
...
*图源：NVIDIA Developer Blog Figure 2，以打印机、挥手人物和地球组成 `print Hello World` 的 rebus。*
...
![RMA 的角色交换与 image token 位置变化](images/14_rma_templates.png)

*图源：论文第 2.1 节，列出默认模板、Role Confusion 和两种 Modality Manipulation 模板。*
...
![SWhisper 的提示优化与近超声注入流程](images/15a_swhisper_workflow.png)

![SWhisper 的近超声播放、麦克风接收与模型响应示意](images/15b_swhisper_setup.png)

*图源：SWhisper 官方项目页的系统总览与攻击场景示意；第一张对应论文工作流，第二张展示扬声器—麦克风—语音模型的物理链路。*
